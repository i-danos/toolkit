...
| Editing a script that is currently executing | bash reads a script incrementally by byte offset, so inserting lines above the running point makes it resume mid-statement. Done to `prep-router.sh` while it was preparing a router — the fix for the entry above, applied on top of itself — and again to `upload.sh` while an upload was in flight, which resumed inside a word and reported `oad: command not found` followed by `ver: unbound variable` at line numbers that are blank in the file you are looking at. That mismatch is the signature: **if the reported line does not match the file, the file changed under the run.** Kill the run, then re-run it; do not assume a long-running script has already been fully read, and do not commit to a script someone may be running. |
