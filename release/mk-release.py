...
VERIFICATION_ITEMS = [
    ...
    ("p1", "drift_event_correlation_broker_dp_transactions", "NOT_RUN", "correlating drift events to brokerd's own transaction boundaries and the dataplane's FAL/DPA transaction completions specifically (distinct from the netlink ingestion side, now covered) -- not started; brokerd's journal has nothing past two startup lines at any log level found so far, and vplsh's own debug/log subcommands did not surface a transaction-level log the way nl_route did for netlink -- would need either raising the dataplane's own --log-level past 8 or reading vplsh source to find an undiscovered subcommand, neither attempted yet"),
