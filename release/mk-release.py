...
VERIFICATION_ITEMS = [
    ...
    ("p1", "comparison_scope_mpls_route", "PASS", "COMPARED in dpa-drift.py now includes mpls-route, keyed by incoming label (zebra 'show mpls table json' vs DPA 'lblspc:0/label:N'). Verified live on the TOPO=fw three-router L3VPN scenario (verify-l3vpn.sh: OSPF+LDP+VPNv4, label 144, cross-VRF ping 5/5): every zebra-installed label (R1 16/17/144, R2 16/17, R3 likewise) matched, no false drift; DPA's reserved labels 0/1/2 are excluded as owned=unknown. Not verified: a *detected* mpls-specific drift -- none was induced, so only the no-false-positive direction is proven for this class."),
    ("p1", "comparison_scope_mcast", "PASS", "COMPARED in dpa-drift.py now includes mroute and mroute6, keyed (source,group) per VRF from 'show ip[v6] mroute vrf all json' against DPA 'vrf:X/(S,G)'. Verified live on the TOPO=fw SSM/IPv6 scenario (verify-multicast-forwarding.sh, 5000 packets each way): R2's (65.1.1.2,232.1.1.1), (2001:db8:65::2,ff0e::1) and (::,ff0e::1) all matched, including zebra's '*' -> '::' any-source mapping, no false drift. Not verified: a *detected* multicast-specific drift (none induced), and the IPv4 any-source spelling ('*' -> 0.0.0.0) was assumed by convention, not observed -- no IPv4 (*,G) existed in this scenario."),
