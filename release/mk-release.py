...
VERIFICATION_ITEMS = [
    ...
    ("p1", "dpa_object_model_interface_vlan", "PASS", "interface class added (vyatta-dataplane 3.14.41): records what creating each router interface returned. Verified live: a VLAN sub-interface (dp0s8.100) is enumerated as if:dp0s8.100, no_support/sw-dataplane, and is still there after a data plane restart. Physical DPDK ports are NOT covered, see dpa_object_model_extension. Not compared against zebra."),
    ("p1", "dpa_object_model_extension", "NOT_RUN", "remaining: physical-port router interfaces (measured: the data plane never calls dpdk_eth_if_l3_enable on these VMs, so no object is recorded for dp0s8/dp0s9 -- cause unresolved), tunnel (GRE path is wired but was not exercised), ACL/firewall, and the two QoS classes, still not_enumerable. vrf, nexthop-group and VLAN interfaces are done."),
