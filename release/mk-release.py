...
VERIFICATION_ITEMS = [
    ...
    ("p2", "power_loss_recovery_upgrade", "FAIL", "verify-power-loss-more.sh MODE=upgrade: qemu SIGKILLed at 13 points across an uninterrupted 31s 'add system image'. 11 of 13 recovered (8 untouched or with an unreferenced partial dir, 3 with the complete new image booting); 2 did not: at 24.7s and 26.3s grub.cfg already named the new image as default while its data was not on disk -- the 26.3s disk stops at 'error: file /boot/upg1/vmlinuz not found ... Press any key' at GRUB, and the same state with a whole kernel and a truncated squashfs stops at an (initramfs) shell ('Can not mount ... .squashfs'). Neither recovers without a console. Root cause: install_image() has no sync before the grub.cfg update (DEFECTS.md 16). Fixed in vyatta-image-tools 5.54 (b8db06c); stays FAIL until an ISO containing 5.54 is built and this sweep is rerun clean through the 24-27s window."),
